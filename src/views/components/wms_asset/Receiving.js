...
export default function Receiving() {
  ...
  useEffect(() => {
    api_get("ReceivingApi").then((data) => setTableData(data));
  }, []);

  const scan = () => {
    api_post("ReceivingApi/scan", { equipment_code: scanCode }).then(() => {
      refreshTable();
    });
  };
  const deleterow = (params) => {
    if (window.confirm("Delete the item?")) {
      api_post("ReceivingApi/delete", params.row).then(() => {
        refreshTable();
      });
    }
  };
  ...
  const refreshTable = () => {
    //reload table
    api_get("ReceivingApi").then((data) => {
      setTableData(data);
  ...
  const search = () => {
    //reload table
    api_get("ReceivingApi", { search_name: searchName }).then((data) =>
      setTableData(data)
    );
  };
